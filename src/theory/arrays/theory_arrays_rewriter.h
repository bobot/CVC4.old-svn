...
public:
  ...
  static RewriteResponse postRewrite(TNode node) {
    Debug("arrays-postrewrite")<<"Arrays::postRewrite start "<<node<<"\n";
    if(node.getKind() == kind::EQUAL || node.getKind() == kind::IFF) {
      ...
      if(node[0].getKind()==kind::SELECT) {
        ...
        if(a.getKind()==kind::STORE) {
          ...
          TNode v = a[2];
          if(v == node[1] && i == j) {
            Debug("arrays-postrewrite")<<"Arrays::postRewrite true \n";
            return RewriteResponse(REWRITE_DONE, NodeManager::currentNM()->mkConst(true));
          }
        }
      }
    ...
    if (node.getKind()==kind::SELECT) {
      ...
      if(a.getKind() == kind::STORE) {
        ...
        TNode v = a[2];
        if(i==j) {
          Debug("arrays-postrewrite")<<"Arrays::postrewrite to "<<v<<"\n";
  ...
  static inline RewriteResponse preRewrite(TNode node) {
    Debug("arrays-prerewrite")<<"Arrays::preRewrite "<<node<<"\n";
    return RewriteResponse(REWRITE_DONE, node);
  }
