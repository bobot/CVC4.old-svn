...
void TheoryEngine::printBenchmark(){
  ...
  if( Trace.isOn("random-benchmark") ){
    ...
    std::cout << "(declare-sort S 0)" << std::endl;
    for( int i=0; i<cardinality; i++ ){
      std::cout << "(declare-fun t_" << i << " () S)" << std::endl;
    }
    std::map< int, std::map< int, bool > > edgesE;
    for( int j=0; j<edges; j++ ){
      int rand1 = (int)(cardinality*double(rand())/double(RAND_MAX));
      int rand2;
      int counter = 0;
      do{
        rand2 = (int)(cardinality*double(rand())/double(RAND_MAX));
        counter++;
      }while( rand1==rand2 || ( counter<500 && edgesE[rand1].find( rand2 )!=edgesE[rand1].end() ) );
      edgesE[ rand1 ][rand2] = true;
      edgesE[ rand2 ][rand1] = true;
      std::cout << "(assert (not (= t_" << rand1 << " t_" << rand2 << ")))" << std::endl;
    }
    std::cout << "(check-sat)" << std::endl;
    exit( 0 );
  }
  Trace("pigeon-benchmark") << std::endl;
  if( Trace.isOn("pigeon-benchmark") ){
    int holes = options::ufssAbortCardinality(); //hack;
    int m = cardinality>holes ? cardinality : holes;
    std::cout << "; Pigeon hole problem for " << cardinality << " " << holes << std::endl;
    std::cout << "(set-logic QF_UF)" << std::endl;
    std::cout << "(declare-sort P 0)" << std::endl;
    std::cout << "(declare-sort H 0)" << std::endl;
    std::cout << "(declare-fun hole (P) H)" << std::endl;
    for( int i=1; i<=m; i++ ){
      if( i<=cardinality ){
        std::cout << "(declare-fun p_" << i << " () P)" << std::endl;
      }
      if( i<=holes ){
        std::cout << "(declare-fun h_" << i << " () H)" << std::endl;
      }
    }
    if( cardinality>1 ){
  ...
  Trace("qf-uf-benchmark") << std::endl;
  Trace("quant-qf-uf-benchmark") << std::endl;
  if( Trace.isOn("quant-qf-uf-benchmark") || Trace.isOn("qf-uf-benchmark") ){
    ...
    if( Trace.isOn("qf-uf-benchmark") ){
      ...
      universalSorts.push_back( defineFuns[0].getType() );
    }
    for( int i=0; i<cardinality; i++ ){
      std::stringstream ss;
      ss << "_c_" << i;
    ...
    if( Trace.isOn("quant-qf-uf-benchmark") ){
