...
void TheoryArith::setupVarList(const VarList& ll){
  Assert(!ll.empty());
  ...
  for(VarList::iterator i = ll.begin(), end = ll.end(); i != end; ++i){
    Variable leaf = *i;
...
void TheoryArith::setupPolynomial(const Polynomial& poly) {
  ...
  for(Polynomial::iterator i = poly.begin(), end = poly.end(); i != end; ++i){
    Monomial mono = *i;
    const VarList& ll = mono.getVarList();
      ...
      if(i != end){
        ...
        if(i == end){
          if(first.getConstant().isOne() && second.getConstant().getValue() == -1){
            VarList ll0 = first.getVarList();
            VarList ll1 = second.getVarList();
...
ArithVar TheoryArith::requestArithVar(TNode x, bool slack){
  Assert(Variable::isMember(x) || x.getKind() == PLUS);
...
void TheoryArith::asVectors(const Polynomial& p, std::vector<Rational>& coeffs, std::vector<ArithVar>& variables) {
  for(Polynomial::iterator i = p.begin(), end = p.end(); i != end; ++i){
    ...
    const VarList& leaf = mono.getVarList();

    Node n = leaf.getNode();

    Assert(Variable::isMember(n));
