...
void ArithUnatePropagator::addImplication(TNode a, TNode b){
  //a and b may be partially simplified
  Node unsimpA = unsimpLit(a);
  Node unsimpB = unsimpLit(b);

  Node imp = NodeBuilder<2>(IMPLIES) << unsimpA << unsimpB;

  Debug("arith-propagate") << "ArithUnatePropagator::addImplication";
  Debug("arith-propagate") << "(" << a << ", " << b <<")" << endl;
  Debug("arith-propagate") << "(" << unsimpA << ", " << unsimpB <<")" << endl;
