...
private:
  Constant constant;
  VarList leafList;
  Monomial(Node n, const Constant& c, const VarList& ll):
  ...
  static Node makeMultNode(const Constant& c, const VarList& ll) {
    Assert(!c.isZero());
    Assert(!c.isOne());
    Assert(!ll.empty());
...
public:
  ...
  Monomial(const Constant& c):
    NodeWrapper(c.getNode()), constant(c), leafList(VarList::mkEmptyVarList())
  { }

  Monomial(const VarList& ll):
    NodeWrapper(ll.getNode()), constant(Constant::mkConstant(1)), leafList(ll)
  {
    Assert( !leafList.empty() );
  }

  Monomial(const Constant& c, const VarList& ll):
  ...
  static Monomial mkMonomial(const Constant& c, const VarList& ll);
  ...
  const VarList& getVarList() const { return leafList; }
