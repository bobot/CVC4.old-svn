...
AntlrSmtParser::AntlrSmtParser(ExprManager* em, pANTLR3_INPUT_STREAM input, const std::string& filename) :
  ...
  pANTLR3_TOKEN_FACTORY pTokenFactory = d_pSmtLexer->pLexer->rec->state->tokFactory;
  if( pTokenFactory != NULL ) {
    pTokenFactory->close(pTokenFactory);
  }

  pTokenFactory = BoundedTokenFactoryNew(d_input, 8);
